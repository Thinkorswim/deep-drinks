...
def transform_data(data, seq_length, vocabulary_size):
    X_limit = limit_vocabulary(data, vocabulary_size=vocabulary_size) # limit the vocabulary size
    X_limit_transformed, label_encoder, onehot_encoder = encode_categorical(X_limit) # one-hot encode the data
...
if __name__ == "__main__":
    ...
    def create_model(params, input_shape, output_units):
        model = Sequential()
        model.add(GRU(256, input_shape=input_shape, activation='relu', return_sequences=True))
